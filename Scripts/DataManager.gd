...
const default_user_data : Dictionary = {"alphabet": {"special": "!?@#£$%&(){}[]<>^|':;", "uppercase": "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "lowercase": "abcdefghijklmnopqrstuvwxyz", "number": "0123456789"}, "autosave": false, "autosave_interval": 1, "autobackup": false, "autobackup_interval": 1, "lastopened": 0, "encryption": false, "case_sensative_search": true, "strict_search": true}
...
func read_data() -> void:
	...
	var passwords_data_exists : bool = (data_exists == true and FileAccess.file_exists(app_path + "/NPS_Data/Passwords.ini"))
	var tags_data_exists : bool = (data_exists == true and FileAccess.file_exists(app_path + "/NPS_Data/Tags.ini"))
	...
	if run_decryption_process:
		while succesfully_read_data == false:
			...
			if passwords_data != [] and tag_data != []:
...
func read_data_file(path : String, data_type : String) -> Array[Dictionary]:
	...
	if len(opened_file_text) < 1 or opened_file_text[0] != "[":
		return to_return
