...
func receive_edit_buttons(button_type : int, id : int) -> void:
	...
	var if_statement : Array[bool]
	var closing_popups_text : Array[String]
	if type == 0:
		if_statement = [active_data["name"] != %TitleContainer/EditTitleField.text, active_data["login"] != %LoginContainer/EditLoginField.text, active_data["password"] != %PasswordGenerationContainer/Row1/NewPasswordField.text]
		closing_popups_text = [("Do You Wish To Change " + str(active_data["name"]) + "'s Name To " + str(%TitleContainer/EditTitleField.text) + "?"), ("Do You Wish To Change " + active_data["name"] + "'s Login From " + active_data["login"] + " To " + %LoginContainer/EditLoginField.text + "?"), ("Do You Wish To Change " + active_data["name"] + "'s Password From " + active_data["password"] + " To " + %PasswordGenerationContainer/Row1/NewPasswordField.text + "?")]
	else:
		if_statement = [active_data["name"] != %TitleContainer/EditTitleField.text]
		closing_popups_text = [("Do You Wish To Change " + str(active_data["name"]) + "'s Name To " + str(%TitleContainer/EditTitleField.text) + "?")]
